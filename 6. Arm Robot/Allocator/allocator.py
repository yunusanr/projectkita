...
def transform_coordinates(x, y):
    if y < 50:
        ...
        map_y = 100 - y
        map_x = 200 - x
        point = np.array([[map_y, map_x]])  # Perhatikan bahwa y datang sebelum x
        ...
        if center_x >= width / 2:
            ...
            if (
                ...
                threading.Thread(
                    ...
                    args=(last_mapped_x, last_mapped_y),
                ).start()
        ...
        if len(approx) == 4:
            ...
            if len(detected_objects) > 0:
                for _, (confidence, box) in detected_objects.items():
                    ...
                    center_y = int((y1 + y2) / 2)
                    mapped_value_x = arduino_map(center_y, 0, height, 200, 0)
                    mapped_dot = arduino_map(center_x, 0, width, 0, 100)

            if arduino_left is None and mapped_dot < 50:
                print("perform 3 dof robot")
                if len(detected_objects) >= 1:
                    semua = []
                    for _, (confidence, box) in detected_objects.items():
                        class_index = int(box.cls[0])
            ...
            if arduino_right is not None and mapped_dot >= 50:
                print("perform 4 dof robot")
                if len(detected_objects) >= 1:
                    semua = []
                    for _, (confidence, box) in detected_objects.items():
                        class_index = int(box.cls[0])
