...
x_start6 = [0, 25, 50, 75, 100, 125, 150, 175, 200]
y_start6 = [0, 12.5, 25, 37.5, 50]  # Lapisan (A-I, J-R, S-AA, AB-AJ, AK-AS)
...
x_start4 = [0, 100, 200]
y_start4 = [0, 25, 50]  # Lapisan (A-I, J-R, S-AA)
...
interp_x4 = RegularGridInterpolator((y_start4, x_start4), x_end4, method="linear")

interp_y4 = RegularGridInterpolator((y_start4, x_start4), y_end4, method="linear")


interp_x6 = RegularGridInterpolator((y_start6, x_start6), x_end6, method="linear")

interp_y6 = RegularGridInterpolator((y_start6, x_start6), y_end6, method="linear")


def transform_coordinates6(x, y):

    point = np.array([[y, x]])  # Perhatikan bahwa y datang sebelum x

    new_x = float(interp_x6(point))

    new_y = float(interp_y6(point))

    return int(new_x), int(new_y)


def transform_coordinates4(x, y):

    point = np.array([[y, x]])  # Perhatikan bahwa y datang sebelum x

    new_x = float(interp_x4(point))

    new_y = float(interp_y4(point))
...
# def get_data_servo_right(x_input, y_input):

#     file_path = "kalibrasiRobot6DOf.xlsx"  # Ganti dengan path file Anda

#     sheet_name = "Sheet1"  # Ganti jika sheet memiliki nama berbeda
...
#     servo1 = data["SERVO1"].values

#     servo2 = data["SERVO2"].values

#     servo3 = data["SERVO3"].values

#     servo4 = data["SERVO4"].values

#     points = list(zip(x_data, y_data))

#     servo1_pred = griddata(points, servo1, (x_input, y_input), method="linear")

#     servo2_pred = griddata(points, servo2, (x_input, y_input), method="linear")

#     servo3_pred = griddata(points, servo3, (x_input, y_input), method="linear")

#     servo4_pred = griddata(points, servo4, (x_input, y_input), method="linear")

#     # Menampilkan hasil prediksi
#     print(f"Koordinat: X={x_input}, Y={y_input}")

#     print(f"Prediksi SERVO1: {servo1_pred}")

#     print(f"Prediksi SERVO2: {servo2_pred}")

#     print(f"Prediksi SERVO3: {servo3_pred}")

#     print(f"Prediksi SERVO4: {servo4_pred}")

#     send_to_arduino_right(servo1_pred, servo2_pred, servo3_pred, servo4_pred)


def send_to_arduino_right(servo1, servo2, servo3, servo4):
...
def send_data_with_feedback_right(mapped_x, mapped_y):
    ...
    # get_data_servo_right(mapped_x, mapped_y)

    wait_for_feedback_right()


# def get_data_servo_left(x_input, y_input):

#     file_path = "kalibrasiRobot4DOf.xlsx"  # Ganti dengan path file Anda
...
#     send_to_arduino_left(servo1_pred, servo2_pred, servo3_pred)


def send_to_arduino_left(servo1, servo2, servo3):
...
def send_data_with_feedback_left(mapped_x, mapped_y):
    ...
    # get_data_servo_left(mapped_x, mapped_y)
    ...
    if not processing_right:  # Only process if not currently processing another object
        ...
        if center_x >= width / 2:
            ...
            if (
                ...
                new_x, new_y = transform_coordinates4(last_mapped_x, last_mapped_y)

                # Use threading to send data without blocking

                threading.Thread(
                    target=send_data_with_feedback_right, args=(new_x, new_y)
                ).start()
