...
x_start1 = [0, 25, 50, 75, 100, 125, 150, 175, 200]
y_start1 = [0, 12.5, 25, 37.5, 50]  # Lapisan (A-I, J-R, S-AA, AB-AJ, AK-AS)
...
interp_x_5dof = RegularGridInterpolator((y_start1, x_start1), x_end1, method="linear")
interp_y_5dof = RegularGridInterpolator((y_start1, x_start1), y_end1, method="linear")
...
interp_x_3dof = RegularGridInterpolator((y_start, x_start), x_end, method="linear")
interp_y_3dof = RegularGridInterpolator((y_start, x_start), y_end, method="linear")


def transform_coordinates3(x, y):
    point = np.array([[y, x]])  # Perhatikan bahwa y datang sebelum x
    new_x = float(interp_x_3dof(point))
    new_y = float(interp_y_3dof(point))
    return int(new_x), int(new_y + 5)


def transform_coordinates5(x, y):
    point = np.array([[y, x]])
    new_x = float(interp_x_5dof(point))
    new_y = float(interp_y_5dof(point))
    print(f"Transform coordinates {new_x},{new_y}")
    return int(new_x + 5), int(new_y)
...
def kanan(warped):
    ...
    if not processing_right:  # Only process if not currently processing another object
        ...
        new_x, new_y = transform_coordinates5(last_mapped_x, last_mapped_y)
        print(
            "test right",
            last_mapped_y,
            new_y,
...
def kirispes(center_x, center_y):
    ...
    if not processing_left:
        ...
        print(f"5 dof send coordinate is {center_x},{center_y}")
        threading.Thread(
...
def kananspes(center_x, center_y):
    ...
    if not processing_right:
        mapped_value_x = arduino_map(center_y, 0, height, 0, 200)
        mapped_value_y = arduino_map(center_x, 280, width, 75, 0)
        print(f"3 dof send coordinate is {center_x},{100 - center_y}")
...
def kiri(warped):
    ...
    if not processing_left:  # Only process if not currently processing another object
        ...
        if center_x < int(width / 2):
            ...
            last_mapped_x = mapped_value_x
            last_mapped_y = mapped_value_y
            new_x, new_y = transform_coordinates3(last_mapped_x, last_mapped_y)
            print(
                "test left",
                last_mapped_y,
                new_y,
...
while True:
    ...
    if contours:
        ...
        if len(approx) == 4:
            ...
            if arduino_left is not None and arduino_right is not None:
                if len(detected_objects) == 1:
                    ...
                    for _, (confidence, box) in detected_objects.items():
                        ...
                        # print(f"Center y is {center_y}")
                        print(f"Mapped y is {mapped_value_y}")
                        print(f"Mapped x is {mapped_value_x}")

                        if mapped_value_y > 50:
                            print("Give coordinate for 5 dof robot")
                            kanan(warped)
                            # kananspes(center_x=center_x, center_y=center_y)

                        else:
                            print("Give coordinate for 3 dof robot")
                            kiri(warped)
                else:
                    continue
            ...
            # if arduino_left is not None:
            #     if len(detected_objects) == 1:
            #         print("object detected only one")
            #         for _, (confidence, box) in detected_objects.items():
            #             # kanan(warped=warped)
            #             kiri(warped=warped)
            #     elif len(detected_objects) > 1:
            #         print("object detected more than one")
            #         jumlahkanan = []
            #         jumlahkiri = []
            #         semua = []
            #         for _, (confidence, box) in detected_objects.items():
            ...
            #             kananspes(center_x=center_x, center_y=center_y)
            #         elif len(semua) > 1:
            #             max_pair = find_max_x_by_color(semua)q
