...
class VehicleDimensionApp:
    def __init__(
        ...
        undetected_frames_limit=4,  # Number of frames to wait before finalizing measurement
    ):
        ...
        self.final_measurement_taken = False
        self.undetected_frames = 0  # Counter for undetected frames
        self.undetected_frames_limit = undetected_frames_limit  # Initialize the limit
    ...
    def predict_and_detect(self, frame):
        ...
        if boxes.size > 0 and clss is not None:
            ...
            if largest_contour is not None:
                ...
                if (
                    not self.final_measurement_taken
                ):  # Only take measurement if not already taken
                    self.total_length += length
                    self.detected_objects.add(name)
                    self.is_vehicle_tracked = True
                    self.previous_box = box  # Store the current box for tracking
                    self.final_measurement_taken = (
                        True  # Set flag to indicate measurement taken
                    )
                    self.undetected_frames = 0  # Reset undetected frames counter

                cv2.drawContours(frame, [largest_contour], -1, (255, 0, 0), 2)

        # If no vehicle is detected, increment the undetected frames counter
        if not detected_this_frame:
            self.undetected_frames += 1
            if self.undetected_frames >= self.undetected_frames_limit:
                self.final_measurement_taken = (
                    False  # Reset the flag when vehicle is not detected for too long
                )
                self.undetected_frames = 0  # Reset the counter
...
if __name__ == "__main__":
    ...
    app = VehicleDimensionApp(video_path, pixel_to_cm=0.1, confidence_threshold=0.2)
