...
class VehicleDimensionApp:
    ...
    def process_video(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            while cap.isOpened():
                ...
                # Show the real-time merged frame
                merged_frame = self.create_merged_frame()
                cv2.imshow("Real-Time Merged Scanned Objects", merged_frame)
    ...
    def create_merged_frame(self):
        if self.scanned_objects:
            # Reverse the order of scanned objects
            reversed_scanned_objects = self.scanned_objects[::-1]

            # Calculate the dimensions for the merged frame
            merged_width = max(mask.shape[1] for mask in reversed_scanned_objects)
            merged_height = sum(mask.shape[0] for mask in reversed_scanned_objects)

            # Create a blank frame to merge scanned objects
            merged_frame = np.zeros((merged_height, merged_width, 3), dtype=np.uint8)

            current_y = 0
            for obj in reversed_scanned_objects:
                # Place each scanned object in the merged frame
        ...
        )  # Return a blank frame if no scanned objects
