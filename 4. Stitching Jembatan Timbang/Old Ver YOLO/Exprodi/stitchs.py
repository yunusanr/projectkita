...
class VehicleDimensionApp:
    def __init__(self, root):
        ...
        # Variables for tracking width
        self.crossed_objects = []  # List to track objects that have crossed the line
        self.accumulated_width = 0  # Accumulated width of objects crossing the line
        self.line_y_position = 240  # Y position of the line for crossing detection
    ...
    def update_image(self):
        while self.running:
            ...
            if boxes.size > 0 and clss is not None:
                for box, cls in zip(boxes, clss):
                    ...
                    if name in [
                        ...
                        x1, y1, x2, y2 = box
                        width = x2 - x1  # Calculate width of the object

                        # Check if the object crosses the line
                        if (
                            y1 < self.line_y_position < y2
                        ):  # If the object crosses the line
                            box_tuple = tuple(
                                box
                            )  # Convert box to tuple for comparison
                            if (
                                box_tuple not in self.crossed_objects
                            ):  # If not already counted
                                self.crossed_objects.append(box_tuple)
                                self.accumulated_width += width  # Accumulate width
            ...
            # Display accumulated width
            self.length_value_label.config(
                text=f"Accumulated Width: {self.accumulated_width} px"
            )
            ...
            cv2.line(
                frame_rgb,
                (0, self.line_y_position),
                (640, self.line_y_position),
                (255, 0, 0),
                1,
            )
            ...
            # Reset accumulation if no objects are detected
            if len(self.crossed_objects) == 0:
                self.accumulated_width = (
                    0  # Reset accumulated width if no objects are crossing
                )
