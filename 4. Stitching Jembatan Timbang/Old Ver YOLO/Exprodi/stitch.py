...
class VehicleDimensionApp:
    def __init__(self, root):
        ...
        # List to store masked frames for stitching
        self.masked_frames = []
        ...
        # Stitching Button
        stitch_button = ttk.Button(
            main_frame, text="Stitch Masked Frames", command=self.stitch_masked_frames
        )
        stitch_button.grid(row=5, column=3, padx=10)
    ...
    def update_image(self):
        while self.running:
            ...
            # Store the masked frame for stitching
            masked_frame_rgb = cv2.cvtColor(masked_frame, cv2.COLOR_BGR2RGB)
            self.masked_frames.append(masked_frame_rgb)

            # Show the masked image in a separate OpenCV window
            cv2.imshow("Masked Objects", masked_frame_rgb)

            # Check for 'q' key to close the masked window
            if cv2.waitKey(1) & 0xFF == ord("q"):
                break

    def stitch_masked_frames(self):
        if len(self.masked_frames) < 2:
            messagebox.showwarning("Warning", "Need at least two frames to stitch.")
            return

        # Create a Stitcher object
        stitcher = cv2.Stitcher_create()
        status, stitched_image = stitcher.stitch(self.masked_frames)
