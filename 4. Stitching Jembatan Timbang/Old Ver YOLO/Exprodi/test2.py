...
class VehicleDimensionApp:
    def __init__(self, video_path, skip_frames=2, resize_factor=0.5):
        ...
        self.detected_bboxes = []  # List to store bounding boxes of detected objects
                ...
                if name in [
                    ...
                    for contour in current_contours:
                        ...
                        if self.check_crossing_line(contour, h):
                            # Check for overlap with previously detected bounding boxes
                            if not self.is_overlapping((x, y, w, h)):
                                self.scanned_objects.append(
                                    mask
                                )  # Save the masked object
                                self.detected_bboxes.append(
                                    (x, y, w, h)
    ...
    def is_overlapping(self, new_bbox):
        # Check if the new bounding box overlaps with any of the previously detected bounding boxes
        new_x, new_y, new_w, new_h = new_bbox
        for x, y, w, h in self.detected_bboxes:
            if not (
                new_x + new_w < x or new_x > x + w or new_y + new_h < y or new_y > y + h
            ):
    ...
    def process_video(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            while cap.isOpened():
                ...
                # Display scanned objects in real-time
                if self.scanned_objects:
                    merged_frame = self.vertical_merge_and_invert()
                    cv2.imshow("Scanned Objects", merged_frame)
    ...
    def vertical_merge_and_invert(self):
        if self.scanned_objects:
            # Calculate total height and maximum width for the merged frame
            total_height = sum(mask.shape[0] for mask in self.scanned_objects)
            max_width = max(mask.shape[1] for mask in self.scanned_objects)

            # Create a blank frame to merge scanned objects
            merged_frame = np.zeros((total_height, max_width, 3), dtype=np.uint8)

            current_y = 0
            for obj in self.scanned_objects:
                obj_height, obj_width = obj.shape[:2]
                # Ensure the object fits within the merged frame
                if current_y + obj_height <= total_height:
                    merged_frame[current_y : current_y + obj_height, 0:obj_width] = obj
                    current_y += obj_height  # Move the y position for the next object
                else:
                    print(
                        "Warning: Object exceeds merged frame height. Skipping object."
                    )

            # Invert the merged result
            merged_frame = cv2.bitwise_not(merged_frame)
