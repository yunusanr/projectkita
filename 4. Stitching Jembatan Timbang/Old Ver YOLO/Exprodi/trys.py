...
class VehicleDimensionApp:
    def __init__(self, root):
        ...
        # Create a PanedWindow to hold the image and segmentation frames
        ...
        # Segmentation Image
        self.segmentation_label = ttk.Label(
            self.paned_window,
            text="Segmentation Placeholder",
            background="#c0c0c0",
            width=40,  # Adjusted width for better fit
            relief=tk.SUNKEN,
        )
        self.paned_window.add(self.segmentation_label)

        # Calculate Button
        calculate_button = ttk.Button(
            main_frame, text="Calculate", command=self.calculate_masking
        )
        calculate_button.grid(row=6, column=0, columnspan=3, pady=(10, 0))
    ...
    def update_image(self):
        while self.running:
            ...
            depth_mask[(depth > 0) & (depth < min_height_object)] = (
                255  # Use min_height_object as the threshold
            )
            ...
            if boxes.size > 0 and clss is not None:
                for box, cls, mask in zip(boxes, clss, masks):
                    ...
                    if name in [
                        ...
                        # Store the mask for segmentation display
                        ...
                        for contour in current_contours:
                            ...
                            if depth_values:
                                ...
                                cv2.putText(
                                    frame,
                                    f"Nearest Dist: {self.height_surface_var.get()-nearest_distance}",
                                    (x, y - 10),
                                ...
                                # Check if the nearest distance is less than the minimum height object
                                if nearest_distance < min_height_object:
                                    # Draw the contour on the frame
                                    cv2.drawContours(
                                        frame, [contour], -1, (255, 0, 0), 2
                                    )  # Blue contours for objects below min height
                                ...
                                self.height_value_label.config(
                                    text=f"{self.height_surface_var.get()-nearest_distance:.2f} mm"
                                )
                                print(
                                    realW,
                                    realH,
            ...
            # Combine segmentation masks for display
            if self.segmentation_masks:
                combined_mask = np.zeros_like(frame)
                for mask in self.segmentation_masks:
                    combined_mask = cv2.addWeighted(combined_mask, 1, mask, 1, 0)
                combined_mask = cv2.addWeighted(combined_mask, 1, contour_frame, 0.5, 0)

                # Display the combined segmentation mask
                combined_mask_rgb = cv2.cvtColor(combined_mask, cv2.COLOR_BGR2RGB)
                combined_mask_rgb = cv2.resize(combined_mask_rgb, (640, 480))
                combined_mask_pil = Image.fromarray(combined_mask_rgb)
                combined_mask_tk = ImageTk.PhotoImage(image=combined_mask_pil)
                self.segmentation_label.config(image=combined_mask_tk)
                self.segmentation_label.image = combined_mask_tk
    ...
    def calculate_masking(self):
        if not self.segmentation_masks:
            messagebox.showwarning(
                "Warning", "No segmentation masks available to calculate."
            )
            return

        # Combine all masks into a single mask
        combined_mask = np.zeros_like(self.segmentation_masks[0])
        for mask in self.segmentation_masks:
            combined_mask = cv2.addWeighted(combined_mask, 1, mask, 1, 0)

        # Apply scaling to the combined mask
        scale_factor = self.scale_var.get()
        scaled_mask = cv2.resize(combined_mask, None, fx=scale_factor, fy=scale_factor)

        # Display the scaled masked frame
        scaled_mask_rgb = cv2.cvtColor(scaled_mask, cv2.COLOR_BGR2RGB)
        scaled_mask_rgb = cv2.resize(scaled_mask_rgb, (640, 480))
        scaled_mask_pil = Image.fromarray(scaled_mask_rgb)
        scaled_mask_tk = ImageTk.PhotoImage(image=scaled_mask_pil)

        # Create a new window to display the scaled mask
        mask_window = tk.Toplevel(self.root)
        mask_window.title("Scaled Mask")
        mask_label = ttk.Label(mask_window, image=scaled_mask_tk)
        mask_label.image = (
            scaled_mask_tk  # Keep a reference to avoid garbage collection
        )
        mask_label.pack()
