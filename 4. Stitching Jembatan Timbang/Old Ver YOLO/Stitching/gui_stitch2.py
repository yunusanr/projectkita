...
class VehicleDimensionApp:
    ...
    def is_entire_vehicle_detected(self, boxes):
        # Check if the bounding boxes cover the entire frame
        frame_area = self.root.winfo_width() * self.root.winfo_height()
        total_area = 0
        for box in boxes:
            x1, y1, x2, y2 = box
            total_area += (x2 - x1) * (y2 - y1)
        return total_area >= frame_area * 0.8  # Adjust threshold as needed
