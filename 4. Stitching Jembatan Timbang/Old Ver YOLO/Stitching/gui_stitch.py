...
class VehicleDimensionApp:
    def __init__(self, root):
        ...
        self.no_detection_timer = None  # Timer for no detection
        self.detection_timeout = 5  # Timeout duration in seconds
    ...
    def update_image(self):
        while self.running:
            ...
            if boxes.size > 0 and clss is not None:
                # Reset the no detection timer
                self.reset_no_detection_timer()
                ...
                # Start the timer for no detection
                self.start_no_detection_timer()
    ...
    def reset_no_detection_timer(self):
        if self.no_detection_timer is not None:
            self.no_detection_timer.cancel()
        self.no_detection_timer = threading.Timer(
            self.detection_timeout, self.show_combined_stitching
        )
        self.no_detection_timer.start()

    def start_no_detection_timer(self):
        if self.no_detection_timer is not None:
            self.no_detection_timer.cancel()

    def show_combined_stitching(self):
        if self.stitched_parts:
            combined_stitching = self.create_combined_stitching()
            cv2.imshow("Combined Stitching", combined_stitching)
